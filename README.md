...
- **نرخ زنده بازار ایران:** بخش قیمت حالا نرخ روزانه طلا، سکه، ارز و رمزارز را از چند منبع زنده می‌گیرد و با هم ترکیب می‌کند (TGJU، بون‌بست، نوبیتکس و کوین‌گکو)؛ هر نرخی که این منابع پاسخ ندهند از صفحه‌های عمومی مرجع بازار ایران — moj3.ir، alanchand.com و isignal.ir — خوانده و با واحد اعلامی همان سایت (تومان/ریال) و بازهٔ معقولِ هر دارایی بررسی می‌شود. کش یک‌دقیقه‌ای، تبدیل خودکار ریال/تومان، و اگر بازار در دسترس نباشد آخرین نرخ ذخیره‌شده با برچسب «قدیمی» نمایش داده می‌شود. جدول زنده داخل پنل (**تنظیمات ← بخش قیمت**) هم دیده می‌شود و از API عمومی `GET /api/rates/live` هم در دسترس است.
...
4. **تحلیل نوسانات و سود/زیان:**
   ...
   - **منبع نرخ‌ها:** ترکیب چند منبع زنده بازار ایران (طلا/سکه، ارز آزاد، تتر و رمزارز) با کش یک‌دقیقه‌ای؛ اگر منابع JSON در دسترس نباشند، جدول قیمت سایت‌های مرجع بازار ایران (moj3.ir، alanchand.com، isignal.ir) خوانده می‌شود و هر عدد پس از تبدیل واحد (تومان/ریال) با بازهٔ معقول همان دارایی سنجیده می‌شود. اگر همه منابع لحظه‌ای قطع باشند، آخرین نرخ معتبر ذخیره‌شده با هشدار «قدیمی بودن» ارسال می‌شود تا جدول هیچ‌وقت خالی نماند.

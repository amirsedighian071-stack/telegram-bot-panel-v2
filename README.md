...
# پنل مدیریت ربات تلگرام — نسخه ۳٫۱
...
<details>
<summary><b>▸ محصولات (کاتالوگ)</b></summary>

<div dir="rtl">

**کاربرد:** تعریف محصول دیجیتال یا فیزیکی برای فروش در ربات.

**گام‌به‌گام:**

1. **کارگاه ربات ← محصولات ← افزودن محصول**.
2. عنوان، توضیح، قیمت (به تومان یا واحد تنظیم‌شده) و عکس را وارد کنید.
3. برای محصول رایگان، قیمت را `0` بگذارید؛ دکمه «دریافت» جای «خرید» می‌آید.
4. در بخش **تحویل**، فایل یا متنی که بعد از پرداخت به کاربر می‌رسد را تعیین کنید.
5. با **دیپ‌لینک محصول** می‌توانید لینک مستقیم خرید بسازید و در کانال بگذارید.

**نکته:** اگر قفل عضویت کانال فعال باشد، کاربر پیش از دریافت محصول باید عضو شود؛ بعد از عضویت، خرید به‌صورت خودکار ادامه پیدا می‌کند.
...
1. ربات را در گروه **ادمین با دسترسی حذف پیام و محدودسازی** کنید.
2. گروه در **کارگاه ربات ← گروه‌ها** کشف می‌شود اما تا **فعال‌سازی دستی**، خاموش می‌ماند.
3. برای هر گروه می‌توانید فعال کنید: خوش‌آمدگویی، ضدلینک، ضداسپم، حذف پیام‌های کانال‌های دیگر، فیلتر کلمه، قفل شبانه.
4. تخلف‌ها با شمارنده ثبت می‌شوند و در صورت عبور از حد، اخطار/سکوت اعمال می‌شود.
...
<details>
<summary><b>▸ حذف فوروارد (Relay)</b></summary>

<div dir="rtl">

این ابزار پیام کانال مبدأ را **بدون برچسب فوروارد** در مقصد بازنشر می‌کند.

1. ربات را در کانال مبدأ و مقصد ادمین کنید.
2. در تب **حذف فوروارد**، مبدأ و مقصد را تعریف کنید.
3. می‌توانید متن جایگزین، امضا یا دکمه دلخواه اضافه کنید.
...
<div align="center">
<img src="assets/readme/mac-portal-fa.webp" alt="مینی‌اپ مشتری" width="820">
</div>

<details>
<summary><b>▸ مینی‌اپ چیست و چطور فعالش کنیم؟</b></summary>

<div dir="rtl">

مینی‌اپ، نسخه گرافیکی فروشگاه و پنل کاربری است که **داخل خود تلگرام** باز می‌شود؛ آدرس آن `https://<آدرس Worker>/portal` است.
...
<details>
<summary><b>▸ تب‌های مینی‌اپ</b></summary>
...
<details>
<summary><b>▸ دستورها و رفتار دکمه‌ها</b></summary>
...
| `/start p_<id>` | ورود مستقیم به صفحه یک محصول (دیپ‌لینک) |
| `/help` | متن راهنما |
| `/lang` | تغییر زبان (در حالت دوزبانه) |
| دکمه پشتیبانی | شروع گفتگو با مدیر |

**رفتار دکمه‌ها (نسخه ۳٫۱):**

- زدن هر دکمه شیشه‌ای، **همان پیام** را با متن و دکمه‌های جدید به‌روز می‌کند؛ چت شلوغ نمی‌شود.
- محتوای تحویلی (فایل، کانفیگ، رسید، اعلان پرداخت) همچنان پیام جدید است تا در تاریخچه چت بماند.
- اگر پیام قبلی قابل ویرایش نباشد (مثلاً خیلی قدیمی یا حاوی رسانه باشد)، سیستم به‌صورت خودکار پیام جدید می‌فرستد و کاربر هیچ خطایی نمی‌بیند.

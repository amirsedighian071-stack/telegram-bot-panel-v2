...
- **نرخ زنده بازار ایران:** بخش قیمت حالا نرخ روزانه طلا، سکه، ارز و رمزارز را از چند منبع زنده می‌گیرد و با هم ترکیب می‌کند؛ کش یک‌دقیقه‌ای، تبدیل خودکار ریال/تومان، و اگر بازار در دسترس نباشد آخرین نرخ ذخیره‌شده با برچسب «قدیمی» نمایش داده می‌شود. جدول زنده داخل پنل (**تنظیمات ← بخش قیمت**) هم دیده می‌شود و از API عمومی `GET /api/rates/live` هم در دسترس است.
...
4. **تحلیل نوسانات و سود/زیان:**
   ...
   - **منبع نرخ‌ها:** ترکیب چند منبع زنده بازار ایران (طلا/سکه، ارز آزاد، تتر و رمزارز) با کش یک‌دقیقه‌ای؛ اگر همه منابع لحظه‌ای قطع باشند، آخرین نرخ معتبر ذخیره‌شده با هشدار «قدیمی بودن» ارسال می‌شود تا جدول هیچ‌وقت خالی نماند.

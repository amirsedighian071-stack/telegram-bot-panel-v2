...
test('private APIs reject unauthenticated clients and never expose credentials',async()=>{
  ...
  const s=await h.api('GET','/settings');assert.equal(s.data.settings.botToken,undefined);assert.equal(JSON.stringify(s).includes('TEST_TOKEN'),false);assert.equal(Object.keys(s.data.settings.purposes).length,12);
});

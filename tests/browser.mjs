...
try {
  ...
  let uploaded = false, delivered;
  await page.route('**/api/media', async route => {
    if (route.request().method() !== 'POST') return route.continue();
    assert(route.request().headers()['content-type'].includes('multipart/form-data'));
    assert(route.request().postDataBuffer().includes(Buffer.from('direct-file.txt')));
    uploaded = true;
    await route.fulfill({ json: { ok: true, data: { media: { id: 'abcdef1234567890', kind: 'document', name: 'direct-file.txt', size: 7, fileId: 'TEST-FILE-ID' } } } });
  });
  ...
  await page.locator('[data-upload="ph-media"]').setInputFiles({ name: 'direct-file.txt', mimeType: 'text/plain', buffer: Buffer.from('payload') });
  ...
  await select('ph-kind', 'photo'); assert.equal(await page.locator('#ph-media').inputValue(), '');
  success('Changing media kind clears incompatible previous uploads');
